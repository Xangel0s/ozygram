...
#[derive(Debug, Serialize)]
struct QueryTranslatorOutput {
    mode: &'static str,
    intent: String,
    query: String,
    results: Vec<QueryTranslatorResult>,
    suggestions: Vec<String>,
}

#[derive(Debug, Serialize)]
struct QueryTranslatorResult {
    kind: String,
    path: Option<String>,
    line: Option<usize>,
    title: String,
...
async fn run_query_translator(connection: &BackendClient, input: Vec<String>, json_output: bool, limit: usize, token_budget: usize) -> anyhow::Result<()> {
...
fn extract_line_range(args: &[String]) -> (Vec<String>, Option<(usize, usize)>) {
...
async fn query_grep_like(connection: &BackendClient, args: &[String], limit: usize) -> anyhow::Result<QueryTranslatorOutput> {
...
async fn query_find_symbol(connection: &BackendClient, args: &[String], limit: usize) -> anyhow::Result<QueryTranslatorOutput> {
...
async fn query_context(connection: &BackendClient, args: &[String], limit: usize) -> anyhow::Result<QueryTranslatorOutput> {
...
async fn query_file(connection: &BackendClient, args: &[String]) -> anyhow::Result<QueryTranslatorOutput> {
...
async fn query_trace_or_tree(connection: &BackendClient, args: &[String], incoming: bool, limit: usize) -> anyhow::Result<QueryTranslatorOutput> {
...
async fn query_architecture(connection: &BackendClient, limit: usize) -> anyhow::Result<QueryTranslatorOutput> {
...
async fn query_doctor(connection: &BackendClient) -> anyhow::Result<QueryTranslatorOutput> {
...
fn query_skills(args: &[String], limit: usize) -> QueryTranslatorOutput {
...
fn print_query_output(output: &QueryTranslatorOutput, json_output: bool, token_budget: usize) -> anyhow::Result<()> {
...
fn compact_text(value: &str, max_chars: usize) -> String { let mut one_line = value.split_whitespace().collect::<Vec<_>>().join(" "); if one_line.len() > max_chars { one_line.truncate(max_chars.saturating_sub(1)); one_line.push('…'); } one_line }

fn short_path(path: &str) -> String { let cwd = std::env::current_dir().ok(); if let Some(cwd) = cwd { if let Ok(rel) = Path::new(path).strip_prefix(cwd) { return rel.to_string_lossy().replace('\\', "/"); } } path.replace('\\', "/") }
